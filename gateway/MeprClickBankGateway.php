...
class MeprClickBankGateway extends MeprBasePayPalGateway {
  ...
  public function __construct() {
    $this->name = __("Clickbank Checkout", 'memberpress');
  ...
  protected function set_defaults() {
    ...
    $this->settings = (object)array_merge(
      array(
        'gateway' => 'MeprClickBankGateway',
        'id' => $this->generate_id(),
        'label' => '',
        'use_label' => true,
        'icon' => MEPR_IMAGES_URL . '/checkout/cards.png',
        'use_icon' => true,
        'desc' => __('Pay via your Clickbank account', 'memberpress'),
        'use_desc' => true,
        'api_username' => '',
        'api_password' => '',
        'signature' => '',
    ...
    if($this->is_test_mode()) {
      ...
      $this->settings->url = 'https://www.paypals.com/cgi-bin/webscr';
  ...
  private function process_ipn() {
    ...
    $refunded_types               = array('refunded','reversed','voided');
    $cancel_sub_types             = array('recurring_payment_profile_cancel', 'subscr_cancel', 'recurring_payment_suspended_due_to_max_failed_payment');

    if( isset($_POST['txn_type']) && in_array(strtolower($_POST['txn_type']), $recurring_payment_txn_types) ) {
      $this->record_subscription_payment();
    }
    elseif( ( isset($_POST['txn_type']) && in_array(strtolower($_POST['txn_type']), $failed_txn_types) ) ||
    ...
    elseif( isset($_POST['parent_txn_id']) && !isset($_POST['txn_type']) ) {
      if( in_array(strtolower($_POST['payment_status']), $refunded_types) ) { return $this->record_refund(); }
    }
  ...
  */
  public function display_payment_page($txn) {
    ...
    if($txn->gateway == $this->id) {
    	...
    	$clickbank = $wpdb->get_row( "SELECT * FROM " .$wpdb->prefix . "mhm_memberpress_clickbank_memberpress_products WHERE memberpress_id=". $id);


    	if ($clickbank) {
    		$site_id = get_option( 'mhm_memberpress_clickbank_site_id' );

    		if (  $site_id != NULL  && $clickbank->sku != NULL )  {
    			$site_id = get_option( 'mhm_memberpress_clickbank_site_id' );
    			wp_redirect( 'https://'.$site_id.'.pay.clickbank.net/?cbitems='.$clickbank->sku );
    			exit();
    		}
 

    	} else {
    		throw new Exception(__('This product not set on clickbank. Please contact administrator', 'memberpress'));
    	}

      //wp_redirect( 'https://bethub.pay.clickbank.net/?cbitems=1' );
      $mepr_options = MeprOptions::fetch();
      $invoice      = $txn->id . '-' . time();
      $useraction = '';
      ...
      if($ack=='SUCCESS' || $ack=='SUCCESSWITHWARNING') {
        ...
        //throw new Exception(__('The connection to Clickbank failed', 'memberpress'));
      }
      //echo "string =>". $this->settings->url;
    }
